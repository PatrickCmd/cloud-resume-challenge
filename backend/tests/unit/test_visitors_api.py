...
class TestTrackVisitor:
    ...
    def test_track_visitor_success(
        self, test_client, override_get_visitor_repository, mock_visitor_repo
    ):
        """Test tracking a visitor."""
        mock_visitor_repo.track_visitor.return_value = {"count": 100, "sessionId": "session-123"}

        track_data = {"page_path": "/blog/my-post", "referrer": "https://google.com"}
    ...
    def test_track_visitor_with_existing_session(
        self, test_client, override_get_visitor_repository, mock_visitor_repo
    ):
        """Test tracking a visitor with existing session cookie."""
        mock_visitor_repo.track_visitor.return_value = {
            "count": 100,
            "sessionId": "existing-session",
        }

        track_data = {"page_path": "/", "referrer": ""}
...
class TestGetDailyTrends:
    ...
    def test_get_daily_trends(
        self, test_client, override_get_visitor_repository, mock_visitor_repo, mock_user_info
    ):
        ...
        with patch("src.dependencies.extract_user_from_token", return_value=mock_user_info):
            response = test_client.get(
                "/visitors/trends/daily?days=7", headers={"Authorization": "Bearer valid-token"}
            )
    ...
    def test_get_daily_trends_without_auth(
        self, test_client, override_get_visitor_repository, mock_visitor_repo
    ):
...
class TestGetMonthlyTrends:
    ...
    def test_get_monthly_trends(
        ...
        with patch("src.dependencies.extract_user_from_token", return_value=mock_user_info):
            response = test_client.get(
                "/visitors/trends/monthly?months=6", headers={"Authorization": "Bearer valid-token"}
            )
