...
class VisitorRepository(BaseRepository):
    ...
    def track_visitor(self, session_id: str) -> Dict[str, Any]:
        ...
        tomorrow_midnight = int((datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=2)).timestamp())
        ...
        self.put_item({
            ...
            'Data': {
                ...
                'lastTrackedTime': datetime.now(timezone.utc).isoformat()
            },
        ...
        self.update_item(
            ...
            expression_attribute_values={
                ...
                ':empty_data': {'totalCount': 0, 'lastUpdated': datetime.now(timezone.utc).isoformat()}
            },
            expression_attribute_names={
                '#data': 'Data'
            }
    ...
    def get_daily_trends(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        count_map = {
            item.get('Data', {}).get('date'): item.get('Data', {}).get('count', 0)
            for item in items
        }
    ...
    def get_monthly_trends(self, months: int = 6) -> List[Dict[str, Any]]:
        ...
        for i in range(months):
            ...
            trends.insert(0, {  # Insert at beginning for chronological order
                'month': year_month,
                'visitors': total
            })
